...
class GenerationMixin:
    ...
    def _get_generation_mode(
        ...
        if generation_config.student_lm is not None:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            if generation_config.student_lm is True: # fuck model_kwargs
                return model_kwargs
            else:
            ...
            return self.teacher_student(
                ...
                synced_gpus=synced_gpus,
                sequential=generation_config.low_memory,
                student_lm=generation_config.student_lm,
                student_cf=generation_config.student_cf,
                student_th=generation_config.student_th,
    ...
    def teacher_student(
        ...
        synced_gpus: bool = False,
        sequential: Optional[bool] = None,
        student_lm: "PreTrainedModel" = None,
        student_cf: float = 1.0,
        student_th: float = 0.1,
        student_model_kwargs: Optional[dict] = None,
        **model_kwargs,
    ) -> Union[GenerateBeamOutput, torch.LongTensor]:
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_inputs_student = student_lm.prepare_inputs_for_generation(input_ids, **student_model_kwargs)
            ...
            if sequential:
                if any(
                        model_name in self.__class__.__name__.lower()
                        for model_name in
                        ["fsmt", "reformer", "bloom", "ctrl", "gpt_bigcode", "transo_xl", "xlnet", "cpm"]
                ):
                    raise RuntimeError(
                        f"Currently generation for {self.__class__.__name__} is not supported "
                        f"for `low_memory beam_search`. Please open an issue on GitHub if you need this feature."
                ...
                inputs_per_sub_batches_student = _split_model_inputs(
                    model_inputs_student, split_size=batch_size, full_batch_size=batch_beam_size
                )
                outputs_per_sub_batch_student = [
                    student_lm(
                        **inputs_per_sub_batch_student,
                        return_dict=True,
                        output_attentions=output_attentions,
                        output_hidden_states=output_hidden_states,
                    )
                    for inputs_per_sub_batch_student in inputs_per_sub_batches_student
                ]
                outputs_student = stack_model_outputs(outputs_per_sub_batch_student)
            ...
            next_token_scores_mask = next_token_scores - student_cf * next_token_scores_student
            next_token_scores_mask[
                next_token_scores < student_th * torch.max(next_token_scores, dim=0)[0]
                ] = float("-inf")
